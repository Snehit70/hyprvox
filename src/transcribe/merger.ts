...
import { GroqMergeClient } from "./merge-client";
import { calculateMergeAccuracy } from "./merge-confidence";
import {
...
export type { FormattingMode } from "./merge-prompts";
export {
	buildMergeUserPrompt,
	calculateMergeMaxTokens,
...
export class TranscriptMerger {
	private readonly mergeClient = new GroqMergeClient();
	private contextLexicon: string[] = [];

	public reset(): void {
		this.mergeClient.reset();
	}
	...
	public async merge(
		...
		try {
			...
			const completion = await withRetry(
				async (signal) => {
					return await this.mergeClient.createCompletionWithFallback(
						apiKey,
						fallbackApiKey,
						{
							model: mergeModel,
		...
		return {
			...
			strategy: mergeStrategy,
			reason: mergeReason,
			accuracy: calculateMergeAccuracy(
				finalText,
				groqText,
				deepgramText,
				sourcesMatch,
...
${deepgramText}
		...
		return {
			...
			accuracy: calculateMergeAccuracy(finalText, groqText, deepgramText),
		};
	}
}
