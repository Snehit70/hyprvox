...
export function formatCombination(keys: Set<string>): string {
  ...
  const hasCtrl = keys.has("LEFT CTRL") || keys.has("RIGHT CTRL") || keys.has("LEFT CONTROL") || keys.has("RIGHT CONTROL") || keys.has("CONTROL");
  const hasAlt = keys.has("LEFT ALT") || keys.has("RIGHT ALT") || keys.has("ALT");
  const hasShift = keys.has("LEFT SHIFT") || keys.has("RIGHT SHIFT") || keys.has("SHIFT");
  const hasMeta = keys.has("LEFT META") || keys.has("RIGHT META") || keys.has("LEFT WIN") || keys.has("RIGHT WIN") || keys.has("META") || keys.has("WIN");
  ...
  for (const key of keys) {
    const isMod = [
      "LEFT CTRL", "RIGHT CTRL", "LEFT CONTROL", "RIGHT CONTROL", "CONTROL",
      "LEFT ALT", "RIGHT ALT", "ALT",
      "LEFT SHIFT", "RIGHT SHIFT", "SHIFT",
      "LEFT META", "RIGHT META", "LEFT WIN", "RIGHT WIN", "META", "WIN"
    ].includes(key);
...
async function interactiveBind(): Promise<string | null> {
  ...
  return new Promise((resolve) => {
    ...
    console.log(colors.dim("The first non-modifier key you press will complete the combination."));
    ...
    listener.addListener((e, downRaw) => {
      ...
      if (e.state === "DOWN") {
        ...
        if (name === "C" && (down["LEFT CTRL"] || down["RIGHT CTRL"] || down["CONTROL"])) {
          finish(null);
          process.exit(0);
        }
  ...
  .action(() => {
    try {
      ...
      if (maskedConfig.apiKeys) {
        if (maskedConfig.apiKeys.groq) maskedConfig.apiKeys.groq = "gsk_****" + String(maskedConfig.apiKeys.groq).slice(-4);
        if (maskedConfig.apiKeys.deepgram) maskedConfig.apiKeys.deepgram = "****" + String(maskedConfig.apiKeys.deepgram).slice(-4);
      }
  ...
  .action(async () => {
    try {
      ...
      if (confirm) {
        ...
        saveConfig(config);
        console.log(`${colors.green("✅")} Hotkey updated to ${colors.bold(hotkey)}`);
      } else {
        console.log(colors.yellow("Changes not saved."));
      }
    } catch (error) {
      console.error(colors.red("\nFailed to bind hotkey:"), (error as Error).message);
      if ((error as Error).message.includes("permissions")) {
        console.log(colors.dim("Try running with higher permissions or ensure your user is in the 'input' group."));
      }
    }
  });
