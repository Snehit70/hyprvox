...
- **Service (`src/daemon/service.ts`)**: The orchestrator. Maintains the state machine and coordinates hardware (audio/keyboard) with remote APIs. Emits `state` events (it is an `EventEmitter`); the app main relays them to the renderer. The overlay derives its waveform from its own Web Audio capture, so the daemon emits no audio-level events.
